...
metadata:
  hermes:
    ...
    upstream_skill: https://github.com/hermes/hermes/blob/main/skills/xurl/SKILL.md
---
...
1. **Run OAuth on the Mac** (api_server host), not on the phone:
   ```bash
...
2. **Never** tell the user to "run this in your local terminal" — especially from Hermes Mobile chat. Phone users cannot run Mac terminal commands. Report progress as agent actions ("Completing X OAuth on your MacBook…") or an honest blocker after browser+terminal attempts fail.

3. **Forbidden wrong flags:** `--skip-browser` does not exist; use `--headless` when the default browser callback fails.

4. After OAuth succeeds: `xurl auth default APP_NAME USERNAME`, then `xurl auth status` and `xurl whoami` before any write/DM action.

5. **Only** escalate to human when: (a) app not registered yet (needs client-id/secret paste), or (b) X developer portal enrollment/billing blocks the app.
...
4. Register the app locally (user runs this):
   ...
   ```
5. **OAuth (agent completes on Igor Mac fleet)** — see **Fleet operator override**. User-only fallback when not on a Mac agent with browser tools:
   ```bash
...
3. If **no app registered** (`xurl auth apps list` empty), stop and direct user to app registration (client-id/secret paste only). If app exists but `oauth2: (none)`, execute **Fleet operator override** — do NOT tell phone user to run terminal commands.
...
- Upstream agent skill: https://github.com/hermes/hermes/blob/main/skills/xurl/SKILL.md
