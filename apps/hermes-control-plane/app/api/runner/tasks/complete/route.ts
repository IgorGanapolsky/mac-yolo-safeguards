...
export async function POST(request: Request) {
  ...
  const payload = await request.json().catch(() => null) as { taskId?: string; leaseToken?: string; result?: string; error?: string } | null;
  ...
  const completed = await completeTask({ owner: `cloud:${runnerId}`, taskId: payload.taskId, leaseToken: payload.leaseToken, result: payload.result, error: payload.error, actorType: "runner" });
