            ...
            {selectedThread && <div className="conversation-history" ref={conversationHistoryRef} data-testid="conversation-history">
              {threadDetails && threadTimeline.length ? threadTimeline.flatMap((item, index) => {
                ...
                return [
                  ...
                  task.result ? <article key={`task-result-${task.id || index}`} className="conversation-message role-assistant" data-testid="conversation-assistant-result"><span>{taskReceiptLabel(task)}</span><ConversationMeta meta={taskOutputMeta(task)} /><pre>{task.result}</pre><TurnStatusline engine={task.deviceName || (task.route === "cloud" ? "Fenced VPS · Ollama (localhost:11434)" : "Ollama (http://localhost:11434/v1/models)")} ttft={task.completedAt && task.createdAt ? latency(task.completedAt - task.createdAt) : "<10ms"} cost="$0.00" />{feedbackControls(task.id)}</article>
