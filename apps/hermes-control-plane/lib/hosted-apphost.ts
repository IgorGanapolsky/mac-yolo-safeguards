...
export function hostedConnectionCopy(input: {
  ...
  if (live) {
    return {
      ...
      body: "ThumbGate runs on a fenced VPS — $10/mo. Sensitive actions fail closed; this dashboard does not yet expose an approval inbox. The runner and model are healthy.",
      badge: "verified",
      live: true,
      trust,
      advertisePaid,
  ...
  const product = "Hosted Hermes is on a fenced VPS ($10/mo). Sensitive actions fail closed; this dashboard does not yet expose an approval inbox.";
