...
describe("hideDuplicateTaskList", () => {
  ...
  it("keeps the card dump visible for Lessons deep-links (?task=)", () => {
    expect(
      hideDuplicateTaskList({
...
describe("mergeThreadTimeline", () => {
  ...
  it("drops a completed web task whose prompt already appears as a snapshot user bubble", () => {
    const timeline = mergeThreadTimeline({
      snapshot: [{ role: "user", content: "same prompt", createdAt: 200 }],
      tasks: [{ id: "dup", prompt: "same prompt", createdAt: 200, status: "completed" }],
    });
    expect(timeline).toHaveLength(1);
    expect(timeline[0].kind).toBe("snapshot");
  });

  it("keeps an in-flight web task even when the synced bubble already shows the prompt", () => {
    const timeline = mergeThreadTimeline({
      snapshot: [{ role: "user", content: "hello", createdAt: 200 }],
      tasks: [{ id: "pending", prompt: "hello", createdAt: 210, status: "cloud_pending" }],
    });
    expect(timeline.map((item) => (item.kind === "task" ? item.task.id : item.message.content))).toEqual([
      "hello",
      "pending",
    ]);
  });
