...
describe("fenced task leases", () => {
  ...
  it("lets cloud reclaim a task the Mac abandoned mid-execution, not just one never claimed locally", async () => {
    ...
    // lease) could never be picked up by anyone but that same Mac reconnecting.
    ...
    const select = mocks.state.selects.at(-1)!;
    expect(select.sql).toContain("k.status = 'running' AND d.failover_mode = 'auto'");
