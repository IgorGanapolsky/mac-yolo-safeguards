...
test("user bubble does not dump cloud_pending next to SENT", () => {
  ...
  assert.match(dashboard, /pendingWaitCopy\(\)/);
  assert.match(dashboard, /data-testid="conversation-pending"/);
});

test("leash panel is a compact run-target + account strip, not a legal dump", () => {
  ...
  assert.match(leash[0], /className="account-recovery"/);
