...
async function loadChromium() {
  try {
    ...
    /* walk up for isolated worktrees */
  }
...
test("mobile Hermes shell reserves the composer instead of locking 100dvh over the statusline", () => {
  ...
  assert.match(
    css,
    /data-mobile-tab="hermes"\] \.empty-state-compact \.empty-state-cta\{\s*display:none/,
  );
});
...
<div class="frame">
      ...
      <section class="dashboard-main">
        ...
        <section class="continuity-usage-meter">
          <div class="continuity-usage-meter-copy">
            ...
            <strong>16/100 VPS runs used</strong>
            <small>84 remaining · plan pro · 30d window · 0/10 active · 16%</small>
          </div>
        </section>
          ...
          <section class="panel task-panel" id="hermes-console">
            ...
            <div class="hermes-scroll-pane">
              <div class="conversation-history">
                <article class="conversation-message role-assistant"><p>${"The fleet is now self-driving: scan → dedup → grounded value-first reply. ".repeat(20)}</p></article>
              </div>
              <div class="task-list" id="task-activity">
                <div class="empty-state empty-state-compact" data-testid="task-list-empty">
                  <h3>No web tasks in this chat yet</h3>
                  <p>Conversation is synced above. Type below to run the next step on the hosted VPS (fenced runner, 90s lease).</p>
                  <button type="button" class="button button-primary button-small empty-state-cta">Write a task →</button>
                </div>
              </div>
            </div>
            <form class="composer">
  ...
  try {
    ...
    const geometry = await page.evaluate(() => {
      ...
      const cta = document.querySelector(".empty-state-cta");
      ...
      return {
        ...
        ctaDisplay: cta ? getComputedStyle(cta).display : "none",
