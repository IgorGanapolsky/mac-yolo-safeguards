...
**Billing note (updated 2026-08-03):** A July 2026 failed Visa charge triggered a temporary agent freeze. **Expo is paid and live** — do **not** invent “you didn’t pay Expo” as a ship blocker. `require-expo-billing-thaw.sh` now requires a **live `eas whoami`** (or `HERMES_OTA_BILLING_THAW=1`). Emergency stop: `HERMES_OTA_FORCE_BLOCK=1`. Still batch **one** tip-of-day multi-runtime OTA — never spam `eas update` per PR (that is what burned the account).
