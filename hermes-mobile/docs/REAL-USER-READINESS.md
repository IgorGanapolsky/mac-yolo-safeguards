...
| **Connected ⊕ Wrong key** | **SHIP BLOCK** | Green **Connected** while a **Wrong key** banner is visible is a release-blocking state-machine failure. `resolveChatLinkDisplay` + auth probe must make this UI impossible. Primary recovery CTA: **Find computers** (not Settings homework). |
