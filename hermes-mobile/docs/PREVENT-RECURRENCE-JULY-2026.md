...
| S52 | Choose computer claimed **Tailscale is off** while Samsung `tun0` + VPN agent were up (NetInfo wifi primary) (2026-07-30) | Multi-signal `tailscaleVpnDetect` (`hasVpnTransport` + CGNAT); native `HermesTailscaleTunnel` (paid+free MainApplication + R8 keep); **ban absolute off copy** in `computerPickerStatus`; Maestro `tailscale-false-off-ban` tier-0 | `npm test -- --watchman=false src/__tests__/preventRecurrenceContract.test.ts src/__tests__/tailscaleVpnDetect.test.ts src/__tests__/computerPickerStatus.test.ts`; `node scripts/validate-maestro-flows.js` |
| S53 | Empty chat claimed **Tailscale only when away / Wi‑Fi only when local** while Tailscale works at home (2026-07-30) | `ChatEmptyGreeting` honest dual-path copy; **ban** exclusive “when you are away/local” framing in `preventRecurrenceContract` S53 | `npm test -- --watchman=false src/__tests__/ChatEmptyGreeting.test.ts src/__tests__/preventRecurrenceContract.test.ts` |
