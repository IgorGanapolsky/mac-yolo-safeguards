...
| 11 | Bare **Aborted** in chat/banner (2026-07-14) | OpenCode/agent runtime error `Aborted` / AbortError leaked as UI copy | `isRawAbortMessage` + `USER_RUN_INTERRUPTED_MESSAGE` in chatErrors, runProgressDisplay, outbound bubbles, assistant prose | **Unit tests** |
||||||| parent of 7a80d3b8 (fix(pair): never push Wrong-key profiles to the phone)
...
| 13 | Not connected with Tailscale/LAN http (2026-07-14) | Android `cleartextTrafficPermitted=false` + loopback-only domain config blocked `http://100.x` / `192.168.x` while shell curl worked | **base cleartext true** + JS `isPublicHttpUrl` ban for public http; USB reverse still preferred for cable | **OS network_security_config + JS policy** |
