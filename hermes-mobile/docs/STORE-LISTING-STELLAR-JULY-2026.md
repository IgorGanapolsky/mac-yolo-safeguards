...
|---|------|--------|----------|
| 1 | **Submit iOS 1.1 binary** to ASC to replace live 1.0 risky subtitle/keywords | **DONE (in review)** | ASC `1.1` = **WAITING_FOR_REVIEW**, build **16** attached (`c1ab4fcc-…`), releaseType `AFTER_APPROVAL`, safe keywords + subtitle `Control Mac agents from phone`. Live 1.0 still shows locked trademark subtitle until Apple approves. |
| 2 | **Drive ≥10 installs + first review** (no fake reviews) | **PARTIAL** | Review prompt threshold → **1**. TestFlight public link https://testflight.apple.com/join/AgGzdptW. External TF blocked until 1.1 approved (`CLOSED_VERSION`). Play installs≈2; reviews=0. HN/Reddit posts live. |
