...
describe('store listing metadata contract (stellar live)', () => {
  it('Play free/paid + iOS share one product name, never a different brand (e.g. ThumbGate)', () => {
    ...
    expect(playTitle).toBe('Hermes Mobile Agent: AI Leash');
    expect(paidTitle).toBe('Hermes Mobile Agent: AI Leash');
    expect(iosName).toBe('Hermes Mobile Agent: AI Leash');
  ...
  it('iOS subtitle and promo fit limits and avoid review-stale language', () => {
    ...
    expect(keywords).toMatch(/linux,windows,selfhosted,local,developer/i);
