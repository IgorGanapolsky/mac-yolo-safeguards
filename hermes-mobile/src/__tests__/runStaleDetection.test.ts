...
describe('runStaleDetection', () => {
  ...
  it('fails runs with zero output tokens only after 5 minutes', () => {
    const progress = baseProgress({ startedAtMs: 1_000, outputTokens: 0 });
    expect(shouldFailRunAwaitingFirstToken(progress, 1_000 + 5 * 60 * 1000 - 1)).toBe(false);
    expect(shouldFailRunAwaitingFirstToken(progress, 1_000 + 5 * 60 * 1000)).toBe(true);
    expect(msUntilNoTokenFail(progress, 1_000 + 30_000)).toBe(5 * 60 * 1000 - 30_000);
  });
