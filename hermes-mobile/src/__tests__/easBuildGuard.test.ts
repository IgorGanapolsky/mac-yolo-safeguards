...
describe('EAS build guard process contract', () => {
  ...
  it('hard-stops Expo cloud builds during Starter overage window', () => {
    // Keep until-date in the future so this contract does not expire with the
    // calendar (UTC rolled past 2026-07-22 and the fixed fixture went green).
    ...
    expect(result.stderr + result.stdout).toMatch(new RegExp(`HARD-STOPPED until ${until}`));
  });
});
