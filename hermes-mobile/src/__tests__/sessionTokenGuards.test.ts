...
describe('sessionTokenGuards', () => {
  ...
  it('estimates current context as per-call average prompt size', () => {
    // Real dogfood shape: 41 calls totalling 1.36M input = ~33k context (healthy).
    expect(
    ...
    // No api_call_count (older gateway) → no estimate.
  ...
  it('does not flag well-used sessions with a small context', () => {
    // The "constantly see the banner" bug: huge cumulative traffic, tiny context.
  ...
  it('badges recents and forces fresh reopen on BLOCK sessions', () => {
    ...
    expect(shouldSuggestFreshOnSessionSelect({ input_tokens: MEGA_SESSION_TOKEN_WARN })).toBe(true);
    expect(shouldSuggestFreshOnSessionSelect({ input_tokens: MEGA_SESSION_TOKEN_BLOCK })).toBe(false);
