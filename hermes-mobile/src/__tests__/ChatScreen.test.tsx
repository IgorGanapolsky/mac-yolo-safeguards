...
describe('ChatScreen', () => {
  ...
  it('keeps chat available in relay mode when the account is not paired yet', async () => {
    ...
    expect(getByTestId('chat-input')).toBeTruthy();
    expect(getByTestId('chat-context-mac').props.children).toBe('Hermes Relay');
    expect(getByTestId('chat-context-link').props.children).toContain('Pair Hermes Relay');
  });
  ...
  it('Forget this Mac closes picker then confirms before removeGatewayProfile (Alert not swallowed)', async () => {
    ...
    fireEvent.press(getByTestId('chat-context-mac-button'));
    expect(getByTestId('remove-gateway-profile-macmini')).toHaveTextContent(/^Forget$/);
