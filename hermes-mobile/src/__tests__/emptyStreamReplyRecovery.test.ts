...
describe('emptyStreamReplyRecovery', () => {
  ...
  it('hard-stops Checking your Mac forever waits', () => {
    ...
    // DEFECT 2 (2026-07-25): hard-stop copy must not point at Leash with nothing waiting.
    expect(EMPTY_STREAM_HARD_STOP_STATUS.toLowerCase()).not.toContain('leash');
    expect(
      emptyStreamCheckingStatus(3_430_000, { pendingApprovalsCount: 2, macHttpOk: true }).toLowerCase(),
    ).toContain('leash');
