...
describe('ThumbGate promo is a continuity upsell, not a failure fallback', () => {
  it('positions the plans as an additive paid companion on every surface', () => {
    for (const s of SURFACES) {
      ...
      expect(promo.headline).not.toMatch(/ThumbGate(?!\.app)/);
      expect(promo.body).toMatch(/Add a web dashboard and paid Continuity to Hermes Mobile with ThumbGate\.app/);
      ...
      // Never bare "ThumbGate" without .app in promo copy (CEO 2026-07-30).
      expect(promo.buttonLabel).not.toMatch(/ThumbGate(?!\.app)/);
