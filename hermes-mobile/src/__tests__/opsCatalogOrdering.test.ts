import {
  ...
  skillsReadOnlyHint,
  sortCatalogNamesAlphabetically,
...
describe('alphabetical catalog ordering', () => {
  ...
  it('sorts nested tool names A-Z without mutating the gateway response', () => {
    const tools = ['web_search', 'Apple_notes', 'browser_open'];
    expect(sortCatalogNamesAlphabetically(tools)).toEqual([
      'Apple_notes',
      'browser_open',
      'web_search',
    ]);
    expect(tools).toEqual(['web_search', 'Apple_notes', 'browser_open']);
  });
  ...
  it('sorts essential display labels A-Z instead of using a curated product order', () => {
    const { essentials } = require('../utils/opsToolsets').partitionMobileToolsets([
      { name: 'session_search', label: 'Session Search' },
      { name: 'clarify', label: 'Clarifying Questions' },
      { name: 'browser', label: 'Browser' },
    ]);
    expect(essentials.map((toolset: { label: string }) => toolset.label)).toEqual([
      'Browser',
