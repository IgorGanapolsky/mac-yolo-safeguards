...
/**
 * Credential *shapes* for redactor tests — never real secrets.
 * Assembled at runtime so contiguous ghp_/sk-proj_/sk_live_ strings never
 * appear in git (GitGuardian incident 35264537 / commit 924b42f).
 * Bodies use TESTONLY / AWS docs EXAMPLE so scanners and humans see fakes.
 */
// Suffixes stay [A-Za-z0-9] so they match SECRET_PATTERNS (ghp_/sk_live_ disallow _).
const OPENAI_KEY = ['sk', 'proj', 'TESTONLYnotARealOpenAIKeyXX99'].join('-');
const GITHUB_TOKEN = ['ghp', 'TESTONLYnotARealGitHubPat99'].join('_');
const AWS_KEY_ID = ['AKIA', 'IOSFODNN7EXAMPLE'].join(''); // AWS public docs example id
const STRIPE_KEY = ['sk', 'live', 'TESTONLYnotARealStripe99'].join('_');
const CONTACT_EMAIL = ['redact-pii-fixture', 'example.com'].join('@');
