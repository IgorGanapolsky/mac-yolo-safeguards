...
export default function ChatScreen() {
  ...
  const handleStartFreshChat = useCallback(
    ...
    try {
      ...
      if (recentTurns.length === 0) {
        ...
        // Keep recent turns on screen while model context resets.
      ...
      // Preserve typed composer text + recent visible turns (auto-heal must not blank history).
      currentSessionRef.current = null;
  ...
  const confirmMegaSessionSend = useCallback(async (): Promise<'allow' | 'fresh' | 'cancel'> => {
    ...
    // Auto-heal path: migrate draft to a fresh model context without user homework.
    return 'fresh';
  }, []);
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <BottomSheetModal
        ...
        onClose={closeMacPicker}
        contentStyle={styles.macPickerSheetContent}
        testID="mac-picker-modal"
      >
            <View style={styles.macPickerHeader}>
              <Text style={styles.modalTitle}>Choose your computer</Text>
              <TouchableOpacity onPress={closeMacPicker}>
            ...
            >
              <Text
                style={styles.macPickerSubtitle}
                numberOfLines={1}
                ellipsizeMode="tail"
                testID="mac-picker-subtitle"
              >
                {MAC_PICKER_SUBTITLE}
              </Text>
                ...
                onRemove={
                  switchComputerProfiles.length > 1
                    ? (profileId) => {
                        ...
                        confirmForgetGatewayProfileAfterHostDismiss(closeMacPicker, {
                          profileId,
              ...
              <ComputerPickerStatusRegion
                ...
                onExpandHelp={() => setMacPickerHelpExpanded(true)}
                compact={switchComputerProfiles.length > 0}
