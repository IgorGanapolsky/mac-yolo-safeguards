...
1. ~~**No changes needed in `poolside-yolo` itself.**~~ **CORRECTED 2026-07-27 — this item was wrong.** The env vars and subcommands were right, but `--mode allow-all` is **not a real pool mode**. The mode IDs pool actually advertises are `default` / `accept-edits` / `plan` / **`always-allow`**. Because `--mode` silently falls back to `default` ("Always ask") on an unknown ID — printing only `Mode "allow-all" was not found, using the default mode.` — `poolside-yolo`'s interactive path was prompting for every tool while claiming to be autonomous. Fixed in `poolside-yolo` (mode `always-allow`, alias normalization, fail-loud on unknown modes, live `--doctor` probe). **Lesson: this item was verified against Poolside's README prose, not against the running binary — flag values must be proven at runtime, because a wrong one degrades silently instead of erroring.**
...
5. ~~**Keep `POOLSIDE_STANDALONE_MODEL=glm-coding` as-is.**~~ **CORRECTED 2026-07-28 — the model ID was fine; routing *everything* through the gateway was not.** The reasoning ("Laguna S 2.1 is already available via `laguna-free` in the gateway's fallback chain") treated Poolside's model as interchangeable with a route to it, and missed that the two have *different failure domains*. `laguna-free` is OpenRouter's free tier of Laguna; Poolside's own `inference.poolside.ai` is a **separate quota pool** reached with `pool login` credentials. On 2026-07-28 every cloud route in the chain was capped or dead **at the same time** — z.ai weekly cap blown (`glm-coding`/`glm-47` timing out, reset 2026-08-01 21:07), OpenRouter `free-models-per-day` exhausted (`nemotron3-free` 429, `laguna-free` 408), `opencode-go-glm`/`opencode-free` 429, Moonshot suspended for balance. A `glm-coding` request then spent ~40s walking the dead chain and answered as local `qwen3:8b`, or exceeded pool's ACP deadline and surfaced as `429 ... No deployments available for selected model`. `poolside-yolo` was unusable while Igor's own Poolside quota sat untouched. Fixed by adding lanes (`POOLSIDE_YOLO_LANE=auto|native|gateway|local`), with `auto` preferring **native** when `pool login` credentials exist. **Lesson: a fallback chain is only resilient across *independent* failure domains — five routes that all bill through two accounts is one route wearing five hats.**
...
7. **NEW 2026-07-30 — `pool acp` advertises a multimodal capability it cannot honour, and one pasted screenshot permanently bricks a session.** `initialize` returns `promptCapabilities.image: true` while the only models served are `poolside/laguna-s-2.1` / `poolside/laguna-xs-2.1`, both text-only. The TUI therefore accepts an image, inference returns `400 … does not support multimodal (image/video/audio) inputs`, and because `pool` replays the whole conversation each turn the image poisons history — **every later prompt fails too, text-only ones included** (measured: 48 failed turns over 18 hours in one session). Fixed here by `poolside-acp-imageguard`, an ACP proxy that corrects the advertisement and transcribes images to text before they reach the model. Full evidence, reproduction, and the recovery/upstream situation: [`RESEARCH-POOLSIDE-MULTIMODAL-JULY-2026.md`](./RESEARCH-POOLSIDE-MULTIMODAL-JULY-2026.md). **Lesson: same shape as item 1 — a flag/capability that is accepted and looks right while silently doing the wrong thing. Probe the running server, not the prose.**
8. **Nothing to change re: auth.** `pool login` still opens `platform.poolside.ai` for a free API key, and gateway env vars are still correctly withheld from `login`/`update`/`--version`/`--help` in the wrapper so real Poolside auth isn't shadowed.
...
- **`--mode` values** — **corrected 2026-07-27.** The real IDs are `default` (Always ask), `accept-edits` (auto-approve workspace file reads/writes only), `plan` (no mutation), and **`always-allow`** (Allow all — approves every tool call, no prompts). There is **no `allow-all`**. Verified two ways against pool v1.0.14: (a) ACP `session/set_mode` with `always-allow` returns `{}` while `allow-all` returns `session mode "allow-all" is not available; available modes: default (Always ask), accept-edits (Accept edits), plan (Plan), always-allow (Allow all)`; (b) `docs.poolside.ai/cli/interactive-mode` lists the same four IDs. `poolside-yolo` now passes `--mode always-allow`, normalizes the `allow-all`/`yolo` aliases onto it, and refuses to start on an unrecognized mode rather than letting pool fall back to approval prompts.
