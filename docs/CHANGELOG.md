...
- **Active Hermes inference can no longer defeat Mac memory recovery.** A 2026-07-21 incident proved the T-348 guardian could evict Ollama successfully while a live mobile client immediately reloaded the 7.7 GiB model with concurrent 32k-38k-token requests. During the existing bounded recovery window, the guardian now sheds only the exact `ai.hermes.gateway` LaunchAgent when a model remains resident and retries graceful unload; the coordinated watchdog leaves an absent gateway stopped until cooldown expiry, then resumes normal auto-recovery. Normal pressure, unrelated apps, dry-run mode, and healthy on-demand inference are unchanged.
