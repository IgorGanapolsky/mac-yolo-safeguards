...
- **Active Hermes inference can no longer defeat Mac memory recovery.** A 2026-07-21 incident proved the T-348 guardian could evict Ollama successfully while a live mobile client immediately reloaded the 7.7 GiB model with concurrent 32k-38k-token requests. During the existing bounded recovery window, the guardian now disables and boots out only the exact `ai.hermes.gateway` LaunchAgent when a model remains resident, persists a circuit marker, and retries graceful unload. The coordinated watchdog enforces that state against KeepAlive/bootstrap attempts, then re-enables and restores launchd ownership after pressure and cooldown clear without a duplicate manual start. Normal pressure, unrelated apps, dry-run mode, and healthy on-demand inference are unchanged.
- **Self-hosted CI no longer deploys branch guards over the live workstation.** The previous Guard E2E preparation step installed its checked-out `sim-runaway-guard.sh` and `memory-pressure-guardian.sh` directly into live LaunchAgent paths. A concurrent successful run silently rolled the just-verified memory circuit back to an older main copy. CI now verifies live PID-lease support read-only and stages exact branch files under `RUNNER_TEMP`; production guard deployment remains an explicit installer action.
- **Mac mini pressure controls no longer cancel each other out.** The live 2026-07-16 freeze showed the memory guardian correctly detect kernel WARN pressure and 90% swap, but its LaunchAgent passed `--dry-run`; meanwhile the Hermes watchdog re-pinned a 64K Ollama model with `keep_alive=-1` every minute. The guardian now unloads resident models through Ollama's HTTP API, verifies the response, and publishes a shared 10-minute recovery cooldown. The gateway defaults to on-demand loading, uses bounded keep-alive only when explicitly enabled, and skips pin/pre-warm during kernel pressure or cooldown. Ollama and the Hermes gateway are never hard-killed.
- **Persistent Hermes automation Chrome is reclaimed only when abandoned.** Under real pressure the guard can stop the exact `~/.hermes/chrome-cdp-profile` browser after it is old, reparented to launchd, and has no established CDP client. Active CDP sessions and normal Chrome are protected; profile data is preserved.
...
- **Stable freeze-prevention installer and LaunchAgent templates.** `scripts/install-mac-freeze-prevention.sh` installs regular executable copies under `~/.local/bin` and `~/.hermes`, rejects `/private/tmp`, Actions-runner, and dry-run service definitions, and loads the simulator guard, live memory guardian, and on-demand gateway watchdog every 60 seconds.
