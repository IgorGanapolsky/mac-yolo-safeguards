...
/**
 * MongoDB Field CTO (Pete Johnson) Architectural RAG Engine
 * (Retrieval-First, Semantic Agent Cache, Native Reranking, & Precision@K KPIs)
 *
 * Core Pillars from Pete Johnson (MongoDB Field CTO for AI):
 *   1. Hybrid Vector + BM25 Search with Native Cross-Encoder Reranking
 *   2. Agentic Semantic Memory Cache (Instant 0-cost retrieval for repeated queries)
 *   3. Retrieval Quality KPI Tracker (Precision@K & Recall@K metrics)
 */
...
class HybridVectorReranker {
  static rerankCandidates(candidates, query, topK = 3) {
    ...
    const scored = candidates.map((cand) => {
      ...
      // Cross-Encoder Reranking Score formula: 0.6 * Vector + 0.4 * BM25
