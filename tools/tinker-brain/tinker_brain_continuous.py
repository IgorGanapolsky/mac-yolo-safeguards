...
def _read_human_feedback(limit: int = 100) -> list[dict[str, Any]]:
    """Load unconsumed human feedback so LEARN closes the loop (not collect-only)."""
    path = Path.home() / ".hermes" / "receipts" / "tinker-brain" / "feedback.jsonl"
...
def learn_agenda(health: dict[str, Any], funnel: dict[str, Any], billing: dict[str, Any]) -> dict[str, Any]:
    """Fold coverage gaps + funnel fails + human feedback into a durable research/sales agenda."""
    gaps = read_gaps()
    top_gaps = sorted(gaps.items(), key=lambda kv: -kv[1])[:20]
    feedback = _read_human_feedback()
    open_fb = [
        r
        for r in feedback
        if not r.get("consumed") and r.get("signal") in {"thumbs_down", "override", "need_section"}
    ]
    agenda = {
        "updatedAt": _utc(),
        ...
        "humanFeedbackOpen": [
            {
                ...
                "needed_card_section": r.get("needed_card_section"),
                "note": r.get("note"),
            }
            for r in open_fb[:10]
        ],
    ...
    # Human feedback closes into agenda (and is marked consumed when section lands).
    for fb in open_fb[:5]:
        section = fb.get("needed_card_section") or "KNOWN_GAPS"
        q = (fb.get("question") or "")[:80]
        agenda["nextActions"].append(
            f"Human feedback ({fb.get('signal')}): update expert card section [{section}] for: {q}"
        )
    ...
    # Mark feedback rows as consumed once they appear on the agenda (loop closed).
    if open_fb:
        try:
            sys.path.insert(0, str(_HERE))
            from tinker_brain_production_metrics import mark_feedback_consumed  # noqa: WPS433

            for fb in open_fb[:5]:
                needle = str(fb.get("needed_card_section") or fb.get("question") or "")[:40]
                if needle:
                    mark_feedback_consumed(matcher=needle, consumed_by="tinker_brain_continuous.learn_agenda")
        except Exception:  # noqa: BLE001 — agenda write must still succeed
            pass
