...
function parseYamlFallback(str) {
  // Simple fallback parser for our bands.yaml format
  const metrics = [];
  ...
  for (const rawLine of lines) {
    ...
    if (rawLine.startsWith('  - id:')) {
      ...
      } else if (line.startsWith('1sigma:')) {
        currentMetric.tiers['1sigma'] = { action: 'log' };
      } else if (line.startsWith('2sigma:')) {
        currentMetric.tiers['2sigma'] = { action: 'diagnose' };
      } else if (line.startsWith('3sigma:')) {
        currentMetric.tiers['3sigma'] = { action: 'propose_and_revert', auto_intent: true };
      }
    }
  }
  return { metrics };
