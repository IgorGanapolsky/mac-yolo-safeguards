 ...
 * 2. Compound AI Workflow DAG Execution (Prompt -> Code -> Tool -> Judge)
 * 3. Multi-Metric Quantitative Evaluators (Exact Match, JSON Schema, Code Eval, LLM-as-a-Judge)
 * 4. Dual-Mode Dispatch: Vellum Cloud API (https://api.vellum.ai) + Zero-Cost Local Engine
 */
...
class VellumAIEngine {
  ...
  /**
   * 1. Interpolates variables into prompt templates ({{ variable_name }})
   */
  ...
  /**
   * 2. Evaluates model output against structured assertion metrics
   */
      ...
      switch (type) {
        ...
        case 'JSON_SCHEMA':
          try {
            ...
            passed = false;
            score = 0;
            message = `Invalid JSON output: ${e.message}`;
          }
          break;
        ...
        case 'CODE_EVAL':
          try {
            if (typeof codePredicate === 'function') {
              ...
              passed = !!res;
              score = passed ? 100 : 0;
              message = passed ? 'Code evaluation predicate passed' : 'Code evaluation predicate returned false';
            } else {
              passed = false;
              score = 0;
        ...
        case 'LLM_JUDGE':
          // Heuristic local judge scoring for truthfulness, structure, and quality
  ...
  /**
   * 3. Executes a multi-step compound workflow DAG
   */
    ...
    for (const step of workflowDefinition.steps || []) {
      ...
      if (type === 'PROMPT_NODE') {
        ...
        stepResult = {
          ...
          // Simulated or local fast inference
          response: step.mockResponse ? step.mockResponse(context) : `[Vellum Engine Response for: ${rendered.slice(0, 40)}...]`
        };
        context[step.outputVar || id] = stepResult.response;
      } else if (type === 'CODE_NODE') {
        if (typeof step.transform === 'function') {
          stepResult = await step.transform(context);
        } else {
          stepResult = context;
        }
  ...
  /**
   * 4. Call Hosted Vellum Cloud API if credentials exist
   */
...
if (require.main === module) {
    ...
    steps: [
      {
        ...
        outputVar: 'analysis',
        mockResponse: (ctx) => JSON.stringify({
          sentiment: 'URGENT',
  ...
  engine.executeWorkflow(sampleWorkflow, {
