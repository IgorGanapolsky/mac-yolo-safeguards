...
function evaluateCase(testCase) {
  ...
  // MRR: is the first hit the right one? Recall only says "somewhere in top-k".
...
function main() {
  ...
  if (args.json) {
    ...
    console.log(
      `RAG retrieval eval: ${passCount}/${results.length} cases pass · mean recall@k=${report.meanRecallAtK} · mean MRR@k=${report.meanMrrAtK} · mean nDCG@k=${report.meanNdcgAtK}`,
    );
    for (const r of results) {
      const mark = r.pass ? 'PASS' : 'FAIL';
      console.log(
        `  [${mark}] ${r.id} recall@${r.k}=${r.recallAtK.toFixed(2)} MRR=${(r.mrrAtK || 0).toFixed(2)} nDCG=${(r.ndcgAtK || 0).toFixed(2)}${r.error ? ` · ${r.error}` : ''}`,
      );
