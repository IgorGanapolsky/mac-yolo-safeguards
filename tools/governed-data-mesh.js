...
function classifyStripeCharges({
  ...
  for (const ch of list) {
    ...
    const paid = ch.paid === true && ch.status === 'succeeded';
    const isOwner = owners.has(email) || !email;
    if (paid) {
      if (isOwner) {
        ownerPaidCents += amount;
        ownerSuccess.push({ id: ch.id, email, amount });
      } else {
        externalPaidCents += amount;
        externalSuccess.push({ id: ch.id, email, amount });
      }
