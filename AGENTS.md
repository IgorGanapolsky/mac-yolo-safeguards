...
- **Squash-merged orphan branches (2026-08-18):** tip is often NOT an ancestor of `main`. Delete remotes via `gh pr list --state merged` + `git push origin --delete <head>` only when no open PR uses that head — never require `merge-base --is-ancestor` for squash cleanup.
